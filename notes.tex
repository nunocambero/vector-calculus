...
We have to remember level curves and level surfaces, and the fact that the gradient is perpendicular to the level curves and level surfaces. The gradient is a vector that points in the direction of the steepest ascent of a function.
...
\begin{center}
    ...
    \begin{tikzpicture}
        ...
        \addplot[domain=0:360, samples=100, thick, color=blue] ({cos(x)}, {sin(x)});
        \addplot[domain=0:360, samples=100, thick, color=red] ({1.4142 * cos(x)}, {1.4142 * sin(x)});
        \addplot[domain=0:360, samples=100, thick, color=green] ({1.67 * cos(x)}, {1.67 * sin(x)});
        \end{axis}
    \end{tikzpicture}
\end{center}
...
\subsection*{Example}
Obtain, graphically, the maximum and minimum of the function $f(x,y,z) = x^2 + y^2 + z^2$, which is the distance from the origin squared, in the curve intersection of the cylinder $x^2 + y^2 = 1$ and $z = y$.
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
    ...
    \begin{tikzpicture}
        ...
        ({1.4142*cos(x)*sin(y)}, {1.4142*sin(x)*sin(y)}, {1.4142*cos(y)});
        % Curve
        \addplot3[
            domain=0:360,
            samples=50,
        ...
        coordinates {(1,0,0) (-1,0,0) (0,1,1) (0,-1,-1)};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        ({1.4142*cos(x)*sin(y)}, {1.4142*sin(x)*sin(y) + 4}, {1.4142*cos(y)});
        % Curve
        \addplot3[
            domain=0:360,
            y domain=0:180,
        ...
        ({2.5857*cos(x)*sin(y)}, {2.5857*sin(x)*sin(y)}, {2.5857*cos(y)});
        \addplot3[
            domain=0:360,
            y domain=0:180,
            samples=50,
        ...
        ({1.7*cos(x)*sin(y)}, {1.7*sin(x)*sin(y)}, {1.7*cos(y)});
        % Points of intersection
        \addplot3[
            only marks,
            mark=*,
        ...
        \node at (axis cs:0,0.25,0.25) {$P$};
        \node at (axis cs:0,2.75,0.25) {$P'$};
        
        \end{axis}
    \end{tikzpicture}
    \label{fig:sphere_plane}
\end{figure}

\subsection*{Exercise}
Find the minimum value of $f(x,y,z) = x^2 + 2y^2 + 3z^2$ subject to the constraint $x + 2y + 3z = 10$.
