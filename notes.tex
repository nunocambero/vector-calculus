...
\[
\text{Let } \epsilon > 0, \quad \text{we must find } \delta > 0 \text{ such that } \left|\sqrt{x^2 + y^2} \sin\left(\frac{1}{x^2 + y^2}\right)\right| < \epsilon
\]
...
Also, for any \( \epsilon > 0 \), there exists \( 0 < \delta' \leq \delta \) such that when \( \|k\| \leq \delta' \):
\[
\frac{\|s(h)\|}{\|k\|} \leq \epsilon.
\]
\[
...
\]
\[
\| s(Dg|_{a} h + r(h)) \| \leq \epsilon \| Dg|_{a} h + r(h) \|
\]

where $\|s(Dg|_{a} h + r(h))\| = s(k)$ 
\[
s(k) \leq \epsilon \left( \| Dg|_{a} \| + 1 \right) \| h \|
\]

Divide by $\| h \|$:
\[
\frac{\| s(Dg|_{a} h + r(h)) \|}{\| h \|} \leq \epsilon \left( \| Dg|_{a} \| + 1 \right)
\]
